...
void distribute_dofs (DoFHandler<2> &dof_handler)
{
  ...
  std::ofstream out2 ("support.gpl");
  out2 << "plot '-' using 1:2 with lines, '-' with labels point pt 2 offset 1,1" << std::endl;
  GridOut().write_gnuplot (dof_handler.get_triangulation(), out2);
  out2 << "e" << std::endl;
  ...
  DoFTools::map_dofs_to_support_points (MappingQ1<2>(),
                                        dof_handler,
                                        support_points);
  DoFTools::write_gnuplot_dof_support_point_info(out2,
                                                 support_points);
  out2 << "e" << std::endl;
}
